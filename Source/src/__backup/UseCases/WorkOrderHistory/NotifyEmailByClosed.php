...
class NotifyEmailByClosed implements INotifyByStatus
{
    public function send(WorkOrderHistory $workOrderHistory)
    {
        try {
            ...
            {
                $to = $emails;
                $subject = str_replace(':number', $workOrderSearch->wo_number, \Config::get('app.wo_email_cerrado_subject'));
                $pathTemplate = 'templates.emails.wo.cerrado';
                $mailFrom = \Config::get('app.mail_from');
                $mailFromHead = \Config::get('app.wo_email_cerrado_from_head');
