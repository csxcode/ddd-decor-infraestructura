...
class NotifyEmailByConfirmed implements INotifyByStatus
{
    public function send(WorkOrderHistory $workOrderHistory)
    {
        try {
            ...
            {
                $to = $emails;
                $subject = str_replace(':number', $workOrderSearch->wo_number, \Config::get('app.wo_email_confirmado_subject'));
                $pathTemplate = 'templates.emails.wo.confirmado';
                $mailFrom = \Config::get('app.mail_from');
                $mailFromHead = \Config::get('app.wo_email_confirmado_from_head');
