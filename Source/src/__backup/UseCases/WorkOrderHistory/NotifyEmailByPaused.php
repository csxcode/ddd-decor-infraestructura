...
class NotifyEmailByPaused implements INotifyByStatus
{
    public function send(WorkOrderHistory $workOrderHistory)
    {
        try {
            ...
            {
                $to = $emails;
                $subject = str_replace(':number', $workOrderSearch->wo_number, \Config::get('app.wo_email_pausado_subject'));
                $pathTemplate = 'templates.emails.wo.pausado';
                $mailFrom = \Config::get('app.mail_from');
                $mailFromHead = \Config::get('app.wo_email_pausado_from_head');
