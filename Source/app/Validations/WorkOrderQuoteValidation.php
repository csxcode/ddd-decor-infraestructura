...
class WorkOrderQuoteValidation extends BaseValidation
{
    ...
    public function update($request, $resource, $woID, $woqID, &$changes)
    {
        ...
        if($role == UserRoleEnum::GESTOR_INFRAESTRUCTURA)
        {
            ...
            if($request->exists('status_id')) {
                ...
                $error_response = $this->checkStatusByRole($resource, $role, ActionEnum::EDIT, 'status_id', $workOrderQuoteClone, $status_id);
                ...
                // Some values are required if pass the previous validation

                // Amount
                $error_response = $this->checkValueMustBeRequired($resource, $amount, 'amount');

                if($error_response)
                    return $error_response;

                // Currency
                $error_response = $this->checkValueMustBeRequired($resource, $currency, 'currency');

                if($error_response)
                    return $error_response;

                // Time Hours
                $error_response = $this->checkValueMustBeRequired($resource, $time_hours, 'time_hours');

                if($error_response)
                    return $error_response;

                // Payment Type
                $error_response = $this->checkValueMustBeRequired($resource, $payment_type, 'payment_type');
            ...
            // ------------------------------------------------------------------
            $excludedFields = ['vendor_id', 'notification'];
    ...
    // ==================== Private Functions ===========================
    private function checkStatusByRole($resource, $role, $action, $field, $workOrderQuote, $newStatusId)
    {
        if ($action == ActionEnum::EDIT)
        {
            if($role == UserRoleEnum::PROVEEDOR)
            {
                if(!($workOrderQuote->quote_status_id == WorkOrderQuoteStatus::STATUS_PENDIENTE))
                {
                    return response()->json([
                        'object' => AppObjectNameEnum::ERROR,
                        'message' => trans('api/validation.work_order_quote.forbidden_because_does_not_pending'),
                        'error_code' => ErrorCodesEnum::not_valid,
                        'error_data' => [
                            'resource' => AppObjectNameEnum::WORK_ORDER,
                            'field' => $field
                ...
                if(!($newStatusId == WorkOrderQuoteStatus::STATUS_COTIZADO))
                {
                    return response()->json([
                        'object' => AppObjectNameEnum::ERROR,
                        'message' => trans('api/validation.work_order_quote.user_cannot_update_entered_status'),
                        'error_code' => ErrorCodesEnum::not_valid,
                        'error_data' => [
                            'resource' => AppObjectNameEnum::WORK_ORDER,
                            'field' => $field
