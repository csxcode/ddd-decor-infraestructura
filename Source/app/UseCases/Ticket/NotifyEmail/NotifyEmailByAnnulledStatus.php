...
class NotifyEmailByAnnulledStatus
{
    ...
    private function send($emails, $ticketViewData, $dataForTemplate): void
    {
        if (count($emails) > 0) {
            $to = $emails;
            $subject = str_replace(':ticket_number', $ticketViewData->ticket_number, \Config::get('app.ticket_email_annulled_subject'));
            $pathTemplate = 'templates.emails.ticket.annulled';
            $mailFrom = \Config::get('app.mail_from');
            $mailFromHead = \Config::get('app.ticket_email_annulled_from_head');
